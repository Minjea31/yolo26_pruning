        ...
        if remain_out_channels == None:
            ...
            if ones:
                ...
                for i in range(selected_weights.size(0)):
                    if torch.all(selected_weights[i] == 0):
            ...
            if ones:
                ...
                selected_bias = torch.where(selected_bias == 0, torch.ones_like(selected_bias), selected_bias)

                # 파라미터로 다시 등록
                self.bn.weight = torch.nn.Parameter(selected_weight)
